...
TEST(test_error_handling, copy_string)
{
  ...
  // normal truncation
  EXPECT_NO_MEMORY_OPERATIONS({ written = __rcutils_copy_string(buffer, 3, "0123456789"); });
  EXPECT_EQ(written, 2u);
  EXPECT_EQ(strnlen(buffer, sizeof(buffer)), 2u);
  EXPECT_STREQ(buffer, "01");

  // normal truncation, 1 short of buffer length
  EXPECT_NO_MEMORY_OPERATIONS({ written = __rcutils_copy_string(buffer, 9, "0123456789"); });
  ...
  EXPECT_NO_MEMORY_OPERATIONS({ written = __rcutils_copy_string(buffer, 9, ""); });
...
TEST(test_error_handling, reverse_str)
{
  ...
  {
    char buffer[] = "reverseme";
    EXPECT_NO_MEMORY_OPERATIONS({ __rcutils_reverse_str(buffer, 3); });
    EXPECT_STREQ(buffer, "vererseme");
  }
  {
    char buffer[] = "doesntmatter";
    EXPECT_NO_MEMORY_OPERATIONS({ __rcutils_reverse_str(buffer, 0); });
...
TEST(test_error_handling, format_error_string)
{
  ...
  EXPECT_NO_MEMORY_OPERATIONS({ __rcutils_format_error_string(&error_string, &error_state); });
}
